...
export const BottomRightProfile: React.FC<BottomRightProfileProps> = ({
  ...
  // If Logged in: Render user profile widget
  if (currentUser) {
    return (
      <div className={isDocked ? "w-full p-2.5 bg-black/40 border-t border-purple-500/15 backdrop-blur-xl shrink-0" : "fixed bottom-3 right-3 z-40 max-w-sm w-auto"}>
        <div className={`bg-[#121622]/90 border border-white/10 rounded-2xl p-2.5 shadow-xl backdrop-blur-2xl flex items-center justify-between gap-2.5 transition-all hover:border-white/20 w-full`}>
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-[10px] text-white/50 font-mono truncate max-w-[70px]">
                ...
                <button
                  ...
                  className="text-[9px] px-1.5 py-0.5 rounded-md bg-white/5 hover:bg-white/15 text-white/70 hover:text-white border border-white/10 flex items-center gap-1 transition-colors shrink-0"
                  title="상태 모드 변경"
                >
  ...
  return (
    <div className={isDocked ? "w-full p-2.5 bg-black/40 border-t border-purple-500/15 backdrop-blur-xl shrink-0" : "fixed bottom-3 right-3 z-40 max-w-sm w-auto"}>
      {isLoginExpanded ? (
        /* Expanded Inline Login Card */
        <div className="w-full bg-[#121622]/95 border border-blue-500/30 rounded-2xl p-3.5 shadow-2xl backdrop-blur-2xl animate-in slide-in-from-bottom-3 duration-200">
          ...
          {loginError && (
            <div className="mb-2.5 p-1.5 bg-red-500/15 border border-red-500/30 rounded-xl flex items-center gap-1.5 text-red-300 text-[11px]">
          ...
          <form onSubmit={handleInlineLogin} className="space-y-2">
            ...
            <div className="flex items-center gap-2 pt-0.5">
              <button
                ...
                className="flex-1 py-1.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl text-xs shadow-md shadow-blue-600/30 border border-blue-400/30 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
              >
                {loginLoading ? (
                  <>
              ...
              <button
                ...
                className="px-2.5 py-1.5 bg-white/5 hover:bg-white/15 text-white/80 hover:text-white rounded-xl text-xs border border-white/10 transition-colors whitespace-nowrap"
              >
                회원가입
              </button>
            </div>
        ...
        <div className="bg-[#121622]/90 border border-white/10 rounded-2xl p-2 shadow-xl backdrop-blur-2xl flex items-center justify-between gap-2 transition-all hover:border-white/20 w-full">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className="px-2 py-1.5 bg-white/5 hover:bg-white/15 text-white/80 hover:text-white rounded-xl text-xs border border-white/10 transition-colors flex items-center gap-1 whitespace-nowrap"
            >
